...
namespace Game.SpriteDeformerToolsEditor
{
    ...
    public class SpriteDeformerBlendShapeEditor : SpriteDeformerWithBaseOfMaterialEditor
    {
        ...
        public override void inspectorWhenSelectPoints()
        {
            ...
            if (spriteDeformerBlendShape.points.Count > 0)
            {
                int selIndex = GUILayout.Toolbar(spriteDeformerBlendShape.points[0].index, strings);
                if (selIndex < 0) selIndex = 0;
                if (selIndex > spriteDeformerBlendShape.countOfShapes)
                    selIndex = spriteDeformerBlendShape.countOfShapes;
