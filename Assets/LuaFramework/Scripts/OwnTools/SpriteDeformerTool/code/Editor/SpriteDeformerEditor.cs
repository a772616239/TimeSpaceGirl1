...
namespace Game.SpriteDeformerToolsEditor
{
    ...
    public class SpriteDeformerEditor : Editor
    {
        ...
        void OnDisable()
        {
            ...
            {
                UnityEditor.Tools.current = userTool;
            }
        }
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            if (UnityEditor.Tools.current != Tool.None)
            {
                userTool = UnityEditor.Tools.current;
            }
        ...
        public void sceneEditor()
        {
            ...
            foreach (SpritePoint p in spriteDeformer.points)
            {
                ...
                Handles.SphereHandleCap(0, pos, Quaternion.identity, 0.1f * HandleUtility.GetHandleSize(pos), EventType.Repaint);
            }
            ...
            foreach (SpritePoint p in selectedPoints)
            {
                ...
                Handles.SphereHandleCap(0, pos, Quaternion.identity, 0.14f * HandleUtility.GetHandleSize(pos), EventType.Repaint);
            }

            if (deformerTool == null) deformerTool = new DeformerTool(this);
            deformerTool.OnSceneGUI();
            if (Event.current.type == EventType.MouseDown && currentTool == CurrentTool.NONE && UnityEditor.Tools.current != Tool.View && Event.current.button == 0)
            {
            ...
            if (currentTool == CurrentTool.SELECT_RECT)
            {
                ...
                foreach (SpritePoint p in spriteDeformer.points)
                {
                    ...
                    if (r.Contains(pScreenPos, true))
                    {
                        ...
                        Handles.SphereHandleCap(0, pos, Quaternion.identity, 0.15f * HandleUtility.GetHandleSize(pos), EventType.Repaint);
                        inRecPoints.Add(p);
                    }
                }
        ...
        protected virtual void inspectorMain()
        {
            ...
            if (newSprite != spriteDeformer.sprite && spriteDeformer.points.Count == 0 && newSprite != null)
            {
        ...
        public Rect RectHandle(Transform owner, Rect rect, Color lineColor, Color dotColor)
        {
            ...
            {
                Handles.DotHandleCap(0, item, owner.rotation, HandleUtility.GetHandleSize(item) * 0.05f,EventType.Repaint);
            }
            return resRect;
        }
