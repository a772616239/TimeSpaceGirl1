...
namespace Game.SpriteDeformerToolsEditor
{
    class UVEditorWindow : EditorWindow, IHasCustomMenu
    {
        ...
        void OnEnable()
        {
            string s = AssetDatabase.FindAssets("alpha_spriteDeformer")[0];

            bavgroundTexture = (Texture)AssetDatabase.LoadAssetAtPath(AssetDatabase.GUIDToAssetPath(s), typeof(Texture));
        }
        void OnGUI()
        {
            ...
            if (!locked && Selection.activeGameObject != null)
            {
                ...
                if (newSel != null)
                {
                    if (newSel != spriteDeformer)
                    {
                        ...
                        resetParam();
                        this.titleContent = new GUIContent(newSel.gameObject.name);
                        Repaint();
                    }
                }
            }
        ...
        void resetParam()
        {
            if ( spriteDeformer == null || (closestPoint != null && !spriteDeformer.points.Contains(closestPoint)))
                closestPoint = null;
            clossesDiveder = null;
            dragPoint = null;
            latestPoint = null;
        ...
        void drawImage()
        {
            ...
            scrollPosition = GUI.BeginScrollView(new Rect(offsetImage.x, offsetImage.y, this.position.width, this.position.height - offsetImage.y), scrollPosition, new Rect(0, 0, tr.width * zoom, tr.height * zoom));
