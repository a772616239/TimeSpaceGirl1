    ...
    public abstract class SpriteDeformer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        protected virtual void generateMeshData(bool new_tringulate, bool new_uv, bool new_points, bool new_color, bool new_normals)
        {
            if (new_points || new_color || new_uv || new_normals)
            {
                ...
                if (new_points) mesh_vertecs = new Vector3[points.Count];
        ...
        public List<SpritePoint> GetconnectedPoint(SpritePoint point, string pointName = "")
        {
        ...
        public void OnAfterDeserialize()
        {
            ...
            for (int i = 0; i < edgeSerializations.Count; i++)
            {
                ...
                if (editorProps.serelizableSelectedPoints != null)
                {
        ...
        public virtual void AddPoint(SpritePoint point, bool autoOffset = false)
        {
            ...
            if (autoOffset)
            {
                ...
                if (tr != null)
                {
                    ...
                    float a = Game.GeometryMath.Triangle.area(u1, u2, u3); if (a == 0f) return;

                    float a1 = Game.GeometryMath.Triangle.area(u2, u3, point.spritePosition) / a; if (a1 < 0) return;
                    float a2 = Game.GeometryMath.Triangle.area(u3, u1, point.spritePosition) / a; if (a2 < 0) return;
                    float a3 = Game.GeometryMath.Triangle.area(u1, u2, point.spritePosition) / a; if (a3 < 0) return;
        ...
        public SpritePoint[] GetTriangleAroundPoint(SpritePoint point)
        {
            for (int i = 0; i < mesh_triangles.Length; i += 3)
            {
                ...
                if (
                     Game.GeometryMath.Triangle.isBelongOriantByClock(
        ...
        private EdgeDivider[] getCutDivider(Vector2 v1, Vector2 v2)
        {
            ...
            foreach (var item in edges)
            {
                ...
                if (
                    Game.GeometryMath.Line.intersection(v1, v2, item.point1.spritePosition, item.point2.spritePosition, out op)
                    )
                {
