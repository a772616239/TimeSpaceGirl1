...
ClimbTowerEliteBattleReportPopup = Inherit(BasePanel)
local this = ClimbTowerEliteBattleReportPopup

--初始化组件（用于子类重写）
function ClimbTowerEliteBattleReportPopup:InitComponent()
...
--绑定事件（用于子类重写）
function ClimbTowerEliteBattleReportPopup:BindEvent()
...
--添加事件监听（用于子类重写）
function ClimbTowerEliteBattleReportPopup:AddListener()
    
end

--移除事件监听（用于子类重写）
function ClimbTowerEliteBattleReportPopup:RemoveListener()
    
end

--界面打开时调用（用于子类重写）
function ClimbTowerEliteBattleReportPopup:OnOpen(...)
    local args = {...}
...
function ClimbTowerEliteBattleReportPopup:OnShow()
    this.Channel:SetActive(false)


    this.reportDataFast = ClimbTowerManager.GetReportDataByDataId(ClimbTowerManager.ReportId.FastTime, ClimbTowerManager.ClimbTowerType.Advance)
    this.ReportFast:SetActive(not not this.reportDataFast)
    this.reportDataLowest = ClimbTowerManager.GetReportDataByDataId(ClimbTowerManager.ReportId.LowestPower, ClimbTowerManager.ClimbTowerType.Advance)
    this.ReportLowest:SetActive(not not this.reportDataLowest)
    this.reportDataMine = ClimbTowerManager.GetReportDataByDataId(ClimbTowerManager.ReportId.Mine, ClimbTowerManager.ClimbTowerType.Advance)
...
function ClimbTowerEliteBattleReportPopup:FillReportUI(go, sData, ReportId)
    ...
    Util.AddOnceClick(LookBtn, function()
        BattleManager.GotoFight(function()
            ...
            -- end
            monsterGroupId = G_VirtualEliteBattle[this.towerTier].Monster
...
--界面关闭时调用（用于子类重写）
function ClimbTowerEliteBattleReportPopup:OnClose()
...
--界面销毁时调用（用于子类重写）
function ClimbTowerEliteBattleReportPopup:OnDestroy()

end

return ClimbTowerEliteBattleReportPopup