...
ClimbTowerEliteRankPopup = Inherit(BasePanel)
local this = ClimbTowerEliteRankPopup

--初始化组件（用于子类重写）
function ClimbTowerEliteRankPopup:InitComponent()
...
--绑定事件（用于子类重写）
function ClimbTowerEliteRankPopup:BindEvent()
...
--添加事件监听（用于子类重写）
function ClimbTowerEliteRankPopup:AddListener()
end

--移除事件监听（用于子类重写）
function ClimbTowerEliteRankPopup:RemoveListener()
end

--界面打开时调用（用于子类重写）
function ClimbTowerEliteRankPopup:OnOpen()
end

--界面打开或者重新打开后，界面刷新时调用（用于子类重写）
function ClimbTowerEliteRankPopup:OnShow()
    --设置自己的排行数据
    this.SetRankingMyItemInfo(this.record,ClimbTowerManager.serverRankData_Advance.myRankInfo)

    this.rankDatas = ClimbTowerManager.GetSortRanks(ClimbTowerManager.ClimbTowerType.Advance)
...
function ClimbTowerEliteRankPopup:RefreshScroll(index)
...
function this.SetRankingItemInfo(root,data)
    this.SetRankingInfo(root,data.rankInfo,data.userName,data.head,data.headFrame,data.level,data.uid)
...
function this.SetRankingMyItemInfo(root,data)
    this.SetRankingInfo(root,data,PlayerManager.nickName,PlayerManager.head,PlayerManager.frame,PlayerManager.level,PlayerManager.uid)
end

--设置排名逻辑
function this.SetRankingInfo(root, data, playerName, playerHead, playerFrame, PlayerLevel,uid)
    ...
    this.playerHead[root]:SetUID(uid)
...
--界面关闭时调用（用于子类重写）
function ClimbTowerEliteRankPopup:OnClose()
...
return ClimbTowerEliteRankPopup