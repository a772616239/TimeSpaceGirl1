...
ClimbTowerEliteUnlockPopup = Inherit(BasePanel)
local this = ClimbTowerEliteUnlockPopup
...
local items = {}
--初始化组件（用于子类重写）
function ClimbTowerEliteUnlockPopup:InitComponent()
...
--绑定事件（用于子类重写）
function ClimbTowerEliteUnlockPopup:BindEvent()
...
--添加事件监听（用于子类重写）
function ClimbTowerEliteUnlockPopup:AddListener()
    
end

--移除事件监听（用于子类重写）
function ClimbTowerEliteUnlockPopup:RemoveListener()
    
end

--界面打开时调用（用于子类重写）
function ClimbTowerEliteUnlockPopup:OnOpen()
    this.climbTowerType = 1
end

--界面打开或者重新打开后，界面刷新时调用（用于子类重写）
function ClimbTowerEliteUnlockPopup:OnShow()
    self:Init()
end

function ClimbTowerEliteUnlockPopup:Init()

    --> 立即获得
    local rewardNowData = RechargeCommodityConfig[10001]
    ...
    for i = 1, #this.repeatItemViews[1] do
        if i <= #rewardNowData.RewardShow then
            this.repeatItemViews[1][i]:OnOpen(false, {rewardNowData.RewardShow[i][1], rewardNowData.RewardShow[i][2]}, 0.9, nil, nil, nil, nil, nil)
    ...
    --> 奖励总览
    local vipData = ClimbTowerManager.GetChallengeConfigVipData()
    local items = {}
    for i = 1, #vipData do
    ...
    for i = 1, #this.repeatItemViews[2] do
        if i <= #sortItems then
            this.repeatItemViews[2][i]:OnOpen(false, {sortItems[i].itemid, sortItems[i].num}, 0.9, nil, nil, nil, nil, sortItems[i].cornerType)
...
--界面关闭时调用（用于子类重写）
function ClimbTowerEliteUnlockPopup:OnClose()
    
end

--界面销毁时调用（用于子类重写）
function ClimbTowerEliteUnlockPopup:OnDestroy()
    this.repeatItemViews = {}
end

return ClimbTowerEliteUnlockPopup