...
local HighMonthCardPage = quick_class("HighMonthCardPage")
local addTimeNum = 30 * 24 * 60 * 60

local monthCardConFig  = ConfigManager.GetConfig(ConfigName.MonthcardConfig)
function HighMonthCardPage:ctor(mainPanel,gameObject)
...
function HighMonthCardPage:OnShow()
...
-- 妖晶数量显示
function HighMonthCardPage:RefreshStoneShow()
    local curAllMonthCardData = OperatingManager.GetMonthCardData()
    --豪华月卡
    local curMonthCardOpenState = curAllMonthCardData[MONTH_CARD_TYPE.LUXURYMONTHCARD] and curAllMonthCardData[MONTH_CARD_TYPE.LUXURYMONTHCARD].endingTime ~= 0
    self.smallDay1.text = GetLanguageStrById(11476) .. monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].Price*10 .. GetLanguageStrById(11477)
    self.smallDay2.text = GetLanguageStrById(11478).. monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].BaseReward[1][2] .. GetLanguageStrById(12635) .. "</color>"
    SetIcon(monthCardConFig[MONTH_CARD_TYPE.MONTHCARD].BaseReward[1][1])

    local MonthCardEndTime= OperatingManager.EndTimejudgment(MONTH_CARD_TYPE.LUXURYMONTHCARD)

    self.btnActivationRedpoint:SetActive(OperatingManager.RefreshMonthCardRedPoint(MONTH_CARD_TYPE.LUXURYMONTHCARD))
    ...
    if not curMonthCardOpenState then
        ...
        self.time.text = ""
        self.progress.text = GetLanguageStrById(12564)..OperatingManager.GetsmonthSaveAmt()*10 .."/"..monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].Price*10
        self.slider.fillAmount = OperatingManager.GetsmonthSaveAmt()/monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].Price
        ...
        local startDate = os.date(GetLanguageStrById(11030),curAllMonthCardData[MONTH_CARD_TYPE.LUXURYMONTHCARD].endingTime)
    ...
    if self.itemList[self.gameObject] then
        self.itemList[self.gameObject]:OnOpen(false, {monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].BaseReward[1][1],monthCardConFig[MONTH_CARD_TYPE.LUXURYMONTHCARD].BaseReward[1][2]}, 1,false,false,false,self.mainPanel.sortingOrder)
...
function HighMonthCardPage:OnHide()
    if self.timer then
        self.timer:Stop()
        self.timer = nil
    end
