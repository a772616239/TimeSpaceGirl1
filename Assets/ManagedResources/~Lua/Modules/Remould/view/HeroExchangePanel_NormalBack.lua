...
function this:InitComponent(gameObject)
    this.gameObject = Util.GetGameObject(gameObject,"Content/HeroExchangePanel_NormalBack/bg")
    ...
    this.goBtn = Util.GetGameObject(this.SelectHeroPanel, "downPanel/GoBtn1")
    ...
    this.ScrollView.gameObject.name = panelName[1]

    --英雄数据
    this.select = Util.GetGameObject(this.gameObject,"Select")
    this.carInfo = Util.GetGameObject(this.gameObject,"CardInfo")
    ...
    this.curName = Util.GetGameObject(this.cur,"name")
...
function this:BindEvent()
    Util.AddClick(this.goBtn, function()
        ...
        if this.returnItem[2] <= BagManager.GetItemCountById(this.returnItem[1]) then
            local dropList = HeroManager.GetHeroReturnItems(selectHeroData,GENERAL_POPUP_TYPE.GeneralPopup_HeroStarBack,1)
            ...
            table.insert(dropList,this.dataSelflist)
            table.insert(dropList, this.dataReturnlist)
            local item = {}
            ...
            UIManager.OpenPanel(UIName.GeneralPopup, GENERAL_POPUP_TYPE.GeneralPopup_HeroStarBack, item, selectData, function (msg)
                ...
                NetManager.HeroStarBackRequest(1, selectData.dynamicId, function (msg)
...
function this.RefreshHelpBtn()
    Util.AddOnceClick(this.HelpBtn, function()
        UIManager.OpenPanel(UIName.HelpPopup,HELP_TYPE.HeroExchange,this.helpPosition.x,this.helpPosition.y+250)
    end)
end
...
function this.SortTypeClick(_sortType)
    ...
    tarHero = HeroManager.GetAllHeroDataMsinusUpWar1(_sortType,1)
...
--设置需要回溯英雄的信息
function this.ShowCurHeroInfo(_heroData)
    ...
    this.curFrame.sprite = Util.LoadSprite(GetHeroQuantityImageByquality(selectData.Quality,selectData.star))
    this.curIcon.sprite = Util.LoadSprite(selectData.icon)
    this.curProIconBg.sprite = Util.LoadSprite(GetQuantityProBgImageByquality(selectData.Quality,selectData.star))
    this.curProIcon.sprite = Util.LoadSprite(GetProStrImageByProNum(selectData.property))
    this.curName:GetComponent("Text").text = GetLanguageStrById(selectData.name)
    this.curLvBg.sprite = Util.LoadSprite(GetQuantityImageByqualityHexagon(selectData.Quality,selectData.star))
    ...
    FindFairyManager.ResetItemView(this.BackDataList, this.BackDataList.transform, itemList, 10, 0.7, sortingOrder, false, item)
end

---转换材料显示
function this.ShowBackData()
    this.Item:SetActive(true)
    local heroExchangeConfigData = ConfigManager.GetConfigDataByDoubleKey("HeroStarBackConfig", "Star", selectData.star, "Country", selectData.heroConfig.PropertyName)
    ...
    this.returnItem = heroExchangeConfigData.Cost
