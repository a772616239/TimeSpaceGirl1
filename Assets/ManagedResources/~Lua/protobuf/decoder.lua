        ...
        if is_packed then
            ...
            return function (buffer, pos, pend, message, field_dict)
                ...
                if value == nil then
                    ...
                    if decode_value == _DecodeSignedVarint64 then
                        field_dict[key]=tonumber(field_dict[key])
                    end
                end
                local endpoint
...
--    wiretype_mask = wire_format.TAG_TYPE_MASK
    ...
    return function (buffer, pos, pend, tag_bytes)
        ...
        --print("wire_type::"..wire_type.."   pos"..pos.."  pend"..pend)
        if pos > pend then
            print("Error:pos > pend")
            return pend
        else
