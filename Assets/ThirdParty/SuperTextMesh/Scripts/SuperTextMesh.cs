		...
		for(int i=0, iL=hyphenedText.Length; i<iL; i++)
		{
			...
			if (reading)
			{
				...
				*/
				...
				if	(CurrentTextInfo.drawAnimData==null || CurrentTextInfo.drawAnimData.animTime == 0f){ //if no animation data, or no animation time, don't do anything
					//Debug.Log("no anim data for " + i);
					Debug.LogError("drawAnimData");
