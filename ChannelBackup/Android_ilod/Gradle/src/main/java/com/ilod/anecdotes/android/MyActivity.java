...
//                sendMessage("结算金额", "" + Double.parseDouble(info.getPrice()));
                ...
                CustomEvent(99,info.getPrice() + "-" + info.getCurrency() + "-" + info.getOrderId());
            }
...
//                sendMessage("取消付款", "用户取消");
    ...
    GamePotAppStatusChannelLoginDialogListener<GamePotUserInfo> onGamePotAppStatusChannelLoginDialogListener = new GamePotAppStatusChannelLoginDialogListener<GamePotUserInfo>() {
        @Override
        public void onExit() {
            // 点击X按钮时处理
            printLog("关闭选择登录窗口");
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                GamePot.getInstance().checkAppStatus(new GamePotAppStatusResultListener() {
                    ...
                    public void onMainternance(GamePotAppStatus status){
                        GamePot.getInstance().showAppStatusPopup(MyActivity.this, status, new GamePotAppCloseListener() {
                            @Override
                            public void onClose() {
                                // TODO : 调用showAppStatusPopup api时,在需要关闭应用程序的情况下调用
                                // TODO : 请终止程序
                                MyActivity.this.finish();
                            }
                ...
                if(lastLoginType != GamePotChannelType.NONE) {
                    // 以最后一次登录的类型登录的方式。
                    GamePotChannel.getInstance().login(MyActivity.this, lastLoginType, onGamePotAppStatusChannelLoginDialogListener);
                }
                else
                {
                    ...
                    GamePotChannel.getInstance().showLoginWithUI(MyActivity.this, builder, onGamePotAppStatusChannelLoginDialogListener);
                }
            }
        });
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(GamePotChannel.getInstance().getLastLoginType() != GamePotChannelType.GUEST) {
                    callUnityFunc("MessageCallback","현재 로그인 중입니다. 게스트 계정만 연동할 수 있습니다.");//当前非游客登录！
                    return;
                }
                ...
                GamePotChannel.getInstance().createLinking(MyActivity.this, gamePotChannelType, new GamePotChannelListener<GamePotUserInfo>() {
                    ...
                    public void onFailure(GamePotError error) {
                        ...
                        printLog("关联失败");
                        callUnityFunc("MessageCallback",error.getMessage());
                    }
                });
            }
        });
    ...
    public  void CustomEvent(int type,String param){
        ...
        switch (type)
        {
            case 0://通用
            {
                switch (param)
                {
                    case "创建角色": adjustEvent = new AdjustEvent("m6a7rm"); break;
                }
            }
            break;
            case 4://玩家升级
                ...
                {
                    case "10":
                        adjustEvent = new AdjustEvent("fpf8m6");
                        break;
                }
                break;
            case 99://充值成功
                {
                    if(param.equals("取消支付")) {
                        ...
                        String price = strArr[0];
                        String currencyType = strArr[1];
                        String order = strArr[2];

                        adjustEvent = new AdjustEvent("scyj0w");
                        adjustEvent.setRevenue(Double.parseDouble(price), currencyType);
